...
def write_cf(cf, outdir, generate_cf_args=None):
    ...
    af = asdf.AsdfFile(tree={'data': cf, 'generate_cf_args': generate_cf_args})
    af.write_to(fn)


def main(
    halo_cat_path, ndens=DEFAULT_NDENS, nthread=DEFAULT_NTHREAD, outdir=DEFAULT_OUTDIR
):
    t0 = perf_counter()
    ...
    generate_cf_args = {
        'halo_cat_path': halo_cat_path,
        'ndens': ndens,
        'nthread': nthread,
    }
    write_cf(cf, outdir, generate_cf_args=generate_cf_args)
