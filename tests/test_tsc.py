...
def test_tsc_single(ngrid, dtype, nthread):
...
@pytest.mark.filterwarnings("ignore:.*dtype")
@pytest.mark.filterwarnings("ignore:.*npartition")
def test_tsc(ngrid, dtype, nthread, save_result=False):
    ...
    with asdf.open(ref_fn) as af:
        savedens = af['pydens'].copy()
        assert np.allclose(dens, savedens, rtol=1e-4, atol=1e-6)
