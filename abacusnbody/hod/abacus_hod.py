...
class AbacusHOD:
    ...
    def __init__(self, sim_params, HOD_params, clustering_params):
        ...
        self.deltacbins = np.linspace(0, 1, 101)
        self.fenvbins = np.linspace(0, 1, 101)
    ...
    def compute_ngal(self, tracers = None):
        """
        ...
        for etracer in tracers.keys():
            ...
            if etracer == 'LRG':
                ngal_dict[etracer] = AbacusHOD._compute_ngal_lrg(
                    ...
                    tracer_hod['Asat'], tracer_hod['Bcent'], tracer_hod['Bsat'], tracer_hod['ic'])
        return ngal_dict

    @staticmethod
    @njit(fastmath = True)
    def _compute_ngal_lrg(logMbins, deltacbins, fenvbins, halo_mass_func,
                   logM_cut, logM1, sigma, alpha, kappa, Acent, Asat, Bcent, Bsat, ic):
        """
        ...
        for i in range(len(logMbins) - 1):
            for j in range(len(deltacbins) - 1):
                for k in range(len(fenvbins) - 1):
                    ...
                    nsat_temp = n_sat_LRG_modified(Mh_temp, logM_cut_temp, 10**logM_cut_temp, M1_temp, sigma, alpha, kappa)
                    ngal_tot += halo_mass_func[i, j, k] * (ncent_temp + nsat_temp)
        return ngal_tot
