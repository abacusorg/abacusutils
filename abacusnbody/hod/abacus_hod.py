...
class AbacusHOD:
    ...
    def staging(self):
        ...
        # ------------------------------------------------------------
        # local env fix:
        # load raw Menv for all halos, rank globally,
        ...
        if self.want_AB and (not self.halo_lc):
            ...
            numslabs_all = len(halo_info_fns)
            ...
            env_Menv = np.empty(Nenv, dtype=np.float64)
            ...
            hfenv_full = calc_fenv_opt(env_Menv, mbins_env, env_mass)

            env_sort = np.argsort(env_id)
            env_id = env_id[env_sort]
            hfenv_full = hfenv_full[env_sort]
...
def calc_fenv_opt(Menv, mbins, halosM):
    ...
    for ibin in numba.prange(len(mbins) - 1):
        ...
        if Nmask > 1:
            ...
            fenv_rank[mmask] = new_fenv_rank / (Nmask - 1) - 0.5
    return fenv_rank
