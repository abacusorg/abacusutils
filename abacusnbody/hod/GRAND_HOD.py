...
def get_random_cen_stellarmass_linearinterpolation(M_h, Mstar_low, Mstar_up,  M_1, M_0, gamma1, gamma2, sigma_c):
    ...
    nbins = 1000
    # stellarmass = np.linspace(Mstar_low,Mstar_up,nbins)
    stellarmass = np.logspace(np.log10(Mstar_low),np.log10(Mstar_up),nbins)
...
def n_sat_CSMF(M_h, Mstar_low, Mstar_up, M_1, M_0, gamma1, gamma2, sigma_c, a1, a2, M2, b0, b1, b2, delta1, delta2):
    ...
    # delta=stellarmass[1:]-stellarmass[:-1]
    # nsat = np.sum(CSMF_sat[:-1]*delta)
    
    # ncen = n_cen_CSMF(M_h, Mstar_low, Mstar_up, M_1, M_0, gamma1, gamma2, sigma_c)
    nsat = 0
...
def gen_cent(pos, vel, mass, ids, multis, randoms, vdev, deltac, fenv, shear,
    LRG_hod_dict, ELG_hod_dict, QSO_hod_dict, CSMF_hod_dict,
    rsd, inv_velz2kms, lbox, want_LRG, want_ELG, want_QSO, want_CSMF, Nthread, origin):
    """
    ...
    if want_LRG:
        ...
        ic_L, alpha_c_L, Ac_L, Bc_L = LRG_hod_dict['ic'], LRG_hod_dict['alpha_c'], LRG_hod_dict['Acent'], LRG_hod_dict['Bcent']
    ...
    if want_ELG:
        ...
        alpha_c_E, Ac_E, Bc_E, Cc_E, ic_E = ELG_hod_dict['alpha_c'], ELG_hod_dict['Acent'], ELG_hod_dict['Bcent'], ELG_hod_dict['Ccent'], ELG_hod_dict['ic']
    ...
    if want_CSMF:
        Mstar_low_C, Mstar_up_C, M_1_C, M_0_C, gamma1_C, gamma2_C, sigma_c_C = CSMF_hod_dict['Mstar_low'], CSMF_hod_dict['Mstar_up'], CSMF_hod_dict['M_1'], CSMF_hod_dict['M_0'], CSMF_hod_dict['gamma_1'], CSMF_hod_dict['gamma_2'], CSMF_hod_dict['sigma_c']
        ic_C, alpha_c_C, Ac_C, Bc_C = CSMF_hod_dict['ic'], CSMF_hod_dict['alpha_c'], CSMF_hod_dict['Acent'], CSMF_hod_dict['Bcent']
    ...
    for tid in numba.prange(Nthread):
        for i in range(hstart[tid], hstart[tid + 1]):
            ...
            if want_QSO:
                ...
                QSO_marker += N_cen_QSO(mass[i], logM_cut_Q_temp, sigma_Q) * ic_Q * multis[i]
    ...
    for tid in numba.prange(Nthread):
        ...
        for i in range(hstart[tid], hstart[tid + 1]):
            if keep[i] == 1:
                ...
                CSMF_mass[j4] = mass[i]
                M_1_C_temp = 10**(np.log10(M_1_C) + Ac_C * deltac[i] + Bc_C * fenv[i])
                CSMF_stellarmass[j4] = get_random_cen_stellarmass_linearinterpolation(mass[i], Mstar_low_C, Mstar_up_C, M_1_C_temp, M_0_C, gamma1_C, gamma2_C, sigma_c_C)
                CSMF_id[j4] = ids[i]
                j4 += 1
    ...
    for tid in numba.prange(Nthread):
        for i in range(int(hstart[tid]), int(hstart[tid + 1])):
            ...
            if np.random.uniform(0,1)<exp_frac:
                ...
                # ind = np.random.choice(np.where(NFW_draw<c[i])[0])
                while (NFW_draw[ind] > c[i]):
                    ind = np.random.randint(0, len(NFW_draw))
...
def gen_sats_nfw(NFW_draw, hpos, hvel, hmass, hid, hdeltac, hfenv, hshear, hvrms, hc, hrvir,
                 LRG_hod_dict, ELG_hod_dict, QSO_hod_dict, CSMF_hod_dict, want_LRG, want_ELG, want_QSO, want_CSMF,
    ...
    if want_CSMF:
        Mstar_low_C, Mstar_up_C, M_1_C, M_0_C, gamma1_C, gamma2_C, sigma_c_C, a1_C, a2_C, M2_C, b0_C, b1_C, b2_C, delta1_C, delta2_C = \
            CSMF_hod_dict['Mstar_low'], CSMF_hod_dict['Mstar_up'], CSMF_hod_dict['M_1'], CSMF_hod_dict['M_0'], CSMF_hod_dict['gamma_1'], \
            CSMF_hod_dict['gamma_2'], CSMF_hod_dict['sigma_c'], CSMF_hod_dict['a_1'], CSMF_hod_dict['a_2'], CSMF_hod_dict['M_2'], \
            CSMF_hod_dict['b_0'], CSMF_hod_dict['b_1'], CSMF_hod_dict['b_2'], CSMF_hod_dict['delta_1'], CSMF_hod_dict['delta_2']
        Ac_C, As_C, Bc_C, Bs_C, ic_C = \
            CSMF_hod_dict['Acent'], CSMF_hod_dict['Asat'], CSMF_hod_dict['Bcent'], \
            CSMF_hod_dict['Bsat'], CSMF_hod_dict['ic']
        ...
        for i in range(hstart[tid], hstart[tid + 1]):
            ...
            if want_QSO:
                ...
                base_p_Q = N_sat_generic(
                    hmass[i], 10**logM_cut_Q_temp, kappa_Q, M1_Q_temp, alpha_Q) * ic_Q
            ...
            if want_CSMF:
                ...
                base_p_C = n_sat_CSMF(hmass[i], Mstar_low_C, Mstar_up_C, M_1_C_temp, M_0_C, gamma1_C, gamma2_C, sigma_c_C, a1_C_temp, a2_C, M2_C, b0_C, b1_C, b2_C, delta1_C, delta2_C) * ic_C
    ...
    = compute_fast_NFW(NFW_draw, hid, hpos[:, 0], hpos[:, 1], hpos[:, 2], hvel[:, 0], hvel[:, 1], hvel[:, 2],
                        hvrms, hc, hmass, hrvir, rd_pos_C, num_sats_C, f_sigv_C, vel_sat, Nthread,
                        exp_frac, exp_scale, nfw_rescale)
    ...
    if want_CSMF:
        ...
        for tid in numba.prange(Nthread):
            for i in range(int(hstart[tid]), int(hstart[tid + 1])):
                M_1_C_temp = 10**(np.log10(M_1_C) + Ac_C * hdeltac[i] + Bc_C * hfenv[i])
                a1_C_temp = a1_C + As_C * hdeltac[i] + Bs_C * hfenv[i]
                stellarmass_C[i] = get_random_sat_stellarmass_linearinterpolation(M_C[i], Mstar_low_C, Mstar_up_C, M_1_C_temp, M_0_C, gamma1_C, gamma2_C, a1_C_temp, s, a2_C, M2_C, b0_C, b1_C, b2_C, delta1_C, delta2_C)
...
def gen_sats(ppos, pvel, hvel, hmass, hid, weights, randoms, hdeltac, hfenv, hshear,
    ...
    LRG_hod_dict, ELG_hod_dict, QSO_hod_dict, CSMF_hod_dict,
    rsd, inv_velz2kms, lbox, Mpart, want_LRG, want_ELG, want_QSO, want_CSMF, Nthread, origin, keep_cent):
    ...
    if want_CSMF:
        Mstar_low_C, Mstar_up_C, M_1_C, M_0_C, gamma1_C, gamma2_C, sigma_c_C, a1_C, a2_C, M2_C, b0_C, b1_C, b2_C, delta1_C, delta2_C = \
            ...
            CSMF_hod_dict['gamma_2'], CSMF_hod_dict['sigma_c'], CSMF_hod_dict['a_1'], CSMF_hod_dict['a_2'], CSMF_hod_dict['M_2'], \
            CSMF_hod_dict['b_0'], CSMF_hod_dict['b_1'], CSMF_hod_dict['b_2'], CSMF_hod_dict['delta_1'], CSMF_hod_dict['delta_2']
        alpha_s_C, s_C, s_v_C, s_p_C, s_r_C, Ac_C, As_C, Bc_C, Bs_C, ic_C = CSMF_hod_dict['alpha_s'], CSMF_hod_dict['s'],\
                         CSMF_hod_dict['s_v'], CSMF_hod_dict['s_p'], CSMF_hod_dict['s_r'], CSMF_hod_dict['Acent'],\
                         CSMF_hod_dict['Asat'], CSMF_hod_dict['Bcent'], CSMF_hod_dict['Bsat'], CSMF_hod_dict['ic']

    # print(Ac_C, As_C, Bc_C, Bs_C)
    ...
    for tid in numba.prange(Nthread): #numba.prange(Nthread):
        for i in range(hstart[tid], hstart[tid + 1]):
            ...
            if want_CSMF:
                M_1_C_temp = 10**(np.log10(M_1_C) + Ac_C * hdeltac[i] + Bc_C * hfenv[i])
                a1_C_temp = a1_C + As_C * hdeltac[i] + Bs_C * hfenv[i]
                base_p_C = n_sat_CSMF(hmass[i], Mstar_low_C, Mstar_up_C, M_1_C_temp, M_0_C, gamma1_C, gamma2_C, sigma_c_C, a1_C_temp, a2_C, M2_C, b0_C, b1_C, b2_C, delta1_C, delta2_C) * weights[i] * ic_C
                if enable_ranks:
...
#     if(0 not in keep):
#         print("Waring: All particles are used for satelites")
#     else:
#         unused_haloes = sum(keep == 0)
#         print("From "+str(H)+" particles we used:",(H-unused_haloes)/H)
    ...
    for tid in numba.prange(Nthread):
        ...
        for i in range(hstart[tid], hstart[tid + 1]):
            if keep[i] == 1:
                ...
                M_1_C_temp = 10**(np.log10(M_1_C) + Ac_C * hdeltac[i] + Bc_C * hfenv[i])
                a1_C_temp = a1_C + As_C * hdeltac[i] + Bs_C * hfenv[i]
                # stellar_mass = get_random_sat_stellarmass_linearinterpolation(hmass[i], Mstar_low_C, Mstar_up_C, M_1_C_temp, M_0_C, gamma1_C, gamma2_C, a1_C_temp, a2_C, M2_C, b0_C, b1_C, b2_C, delta1_C, delta2_C) 
                stellar_mass = get_random_sat_stellarmass(hmass[i], Mstar_low_C, Mstar_up_C, M_1_C_temp, M_0_C, gamma1_C, gamma2_C, a1_C_temp, a2_C, M2_C, b0_C, b1_C, b2_C, delta1_C, delta2_C) 
                # if(j4>16000):
            
                CSMF_stellarmass[j4] = stellar_mass
                CSMF_mass[j4] = hmass[i]
                # CSMF_stellarmass[j4] = hmass[i]
                CSMF_id[j4] = hid[i]
                j4 += 1
        # assert j == gstart[tid + 1]
...
def gen_gals(halos_array, subsample, tracers, params, Nthread, enable_ranks, rsd, verbose, nfw, NFW_draw = None):
    ...
    LRG_dict_cent, ELG_dict_cent, QSO_dict_cent, CSMF_dict_cent, ID_dict_cent, keep_cent = \
    gen_cent(halos_array['hpos'], halos_array['hvel'], halos_array['hmass'], halos_array['hid'], halos_array['hmultis'],
             ...
             LRG_hod_dict, ELG_hod_dict, QSO_hod_dict, CSMF_hod_dict, rsd, inv_velz2kms, lbox, want_LRG, want_ELG, want_QSO, want_CSMF, Nthread, origin)
    ...
    if nfw:
        ...
        LRG_dict_sat, ELG_dict_sat, QSO_dict_sat, CSMF_dict_sat, ID_dict_sat = \
        ...
        gen_sats(subsample['ppos'], subsample['pvel'], subsample['phvel'], subsample['phmass'], subsample['phid'],
                 ...
                 LRG_hod_dict, ELG_hod_dict, QSO_hod_dict, CSMF_hod_dict, rsd, inv_velz2kms, lbox, params['Mpart'],
                 want_LRG, want_ELG, want_QSO, want_CSMF, Nthread, origin, keep_cent[subsample['pinds']])
    if verbose:
