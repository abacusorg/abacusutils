...
def get_tracer_power(tracer_pos, want_rsd, config, want_save=True, save_3D_power=False):
    ...
    if save_3D_power:
        ...
        pk3d, N3d, binned_poles, Npoles, k_avg = calc_pk_from_deltak(tr_field_fft, Lbox, k_bin_edges, mu_bin_edges, field2_fft=None, poles=np.asarray(poles))
    ...
    for i in range(len(keynames)):
        ...
        if save_3D_power:
            ...
            pk3d, N3d, binned_poles, Npoles, k_avg = calc_pk_from_deltak(field_fft_i[f'{keynames[i]}_Re']+1j*field_fft_i[f'{keynames[i]}_Im'], Lbox, k_bin_edges, mu_bin_edges, field2_fft=tr_field_fft, poles=np.asarray(poles))
            pk3d *= field_D[i]
...
def get_recon_power(tracer_pos, random_pos, want_rsd, config, want_save=True, save_3D_power=False, want_load_tr_fft=False):
    ...
    for i in range(len(keynames)):
        ...
        if save_3D_power:
            ...
            # compute power spectrum
            pk3d, N3d, binned_poles, Npoles, k_avg = calc_pk_from_deltak(fields[keynames[i]], Lbox, k_bin_edges, mu_bin_edges, field2_fft=tr_field_fft, poles=np.asarray(poles))
            pk_tr_dict[f'P_kmu_{keynames[i]}_tr'] = pk3d
            pk_tr_dict[f'N_kmu_{keynames[i]}_tr'] = N3d
            pk_tr_dict[f'P_ell_{keynames[i]}_tr'] = binned_poles
            pk_tr_dict[f'N_ell_{keynames[i]}_tr'] = Npoles
