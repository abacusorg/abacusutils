...
def combine_cross_kaiser_spectra(k, spectra_dict, D, bias, f_growth, rec_algo, R, rsd=False):
    ...
    if rec_algo == "recsym":
        if rsd:
            ...
            pk = D * (bias * spectra_dict['P_kmu_delta_tr'] +
...
def combine_kaiser_spectra(k, spectra_dict, D, bias, f_growth, rec_algo, R, rsd=False):
    ...
    if rec_algo == "recsym":
        if rsd:
            pk = D**2 * (2. * bias * f_growth * spectra_dict['P_ell_deltamu2_delta'] +
                         f_growth**2 * spectra_dict['P_ell_deltamu2_deltamu2'] +
                         bias**2 * spectra_dict['P_ell_delta_delta'])
        else:
            pk = D**2 * (2. * bias * f_growth * spectra_dict['P_kmu_deltamu2_delta'] +
        ...
        if rsd:
            ...
            pk = D**2 * (2. * bias * f_eff * spectra_dict['P_kmu_deltamu2_delta'] +
...
def get_poles(k, pk, D, bias, f_growth, poles=[0, 2, 4]):
    ...
    for i, pole in enumerate(poles):
        if pole == 0:
            p_ell[i] = ((1. + 2./3.*beta + 1./5*beta**2)*pk)
        elif pole == 2:
            p_ell[i] = ((4./3.*beta + 4./7*beta**2)*pk)
        elif pole == 4:
            p_ell[i] = ((8./35*beta**2)*pk)
    p_ell *= bias**2 * D**2
    return k, p_ell
...
def read_power_dict(power_tr_dict, power_ij_dict, want_rsd, keynames, poles):
    ...
    print("zeros in the measured power spectra = ", np.sum(pk_tt == 0.), np.sum(pk_ij_zz == 0.), np.sum(pk_ij_zt == 0.))
...
def run_zcv(power_rsd_tr_dict, power_rsd_ij_dict, power_tr_dict, power_ij_dict, config):
    ...
    k, mu, pk_tt_real, pk_ij_zz_real, pk_ij_zt_real, nmodes = read_power_dict(power_tr_dict, power_ij_dict, want_rsd=False, keynames=keynames, poles=poles)
...
def run_zcv_field(power_rsd_tr_fns, power_rsd_ij_fns, power_tr_fns, power_ij_fns, config):
    ...
    pk_nn_proj = project_3d_to_poles(k_bins, pk_nn, Lbox, poles)[0].reshape(len(poles), len(k_binc))/Lbox**3
    pk_zn_proj = project_3d_to_poles(k_bins, pk_zn, Lbox, poles)[0].reshape(len(poles), len(k_binc))/Lbox**3
    del pk_zn; gc.collect() # noqa: E702
    pk_zz_proj = project_3d_to_poles(k_bins, pk_zz, Lbox, poles)[0].reshape(len(poles), len(k_binc))/Lbox**3

    # expand zenbu to 3D power spectrum
    assert np.min(np.diff(k_binc)) == np.max(np.diff(k_binc)), "For custom interpolation, need equidistant k-values"
...
def run_lcv(power_rsd_tr_dict, power_lin_dict, config):
    ...
    if not logk:
        dk = k_bins[1]-k_bins[0]
        assert np.isclose(dk, k_bins[-1]-k_bins[-2]), "Spacing between k_bins is uneven"
    else:
        dk = np.log(k_bins[1]/k_bins[0])
        assert np.isclose(dk, np.log(k_bins[-1]/k_bins[-2])), "Spacing between k_bins is uneven"
    ...
    # get the bias
    bvec_opt = measure_2pt_bias_lcv(k_binc, power_lin_dict, power_rsd_tr_dict, D, f_growth, kmax, want_rsd, rec_algo, R, ellmax=1)
    bias = np.array(bvec_opt['x'])[0]
    print("bias", bias)

    # get linear prediction for the multipoles
    if rec_algo == "reciso":
        S = np.exp(-kth**2*R**2/2.)
        f_eff = f_growth*(1.-S)
    elif rec_algo == "recsym":
        f_eff = f_growth
    ...
    if want_rsd:
        ...
        var_tt_nosn = 2. * (pk_tt_input-shotnoise[0])**2
...
def run_lcv_field(power_rsd_tr_fns, power_lin_fns, config):
    ...
    # get the bias
    bvec_opt = measure_2pt_bias_lcv(k_binc, pk_ij, {'P_ell_tr_tr': pk_tt.reshape(1, len(pk_tt), 1)}, D, f_growth, kmax, want_rsd, rec_algo, R, ellmax=1)
    bias = np.array(bvec_opt['x'])[0]
    print("bias", bias)
    ...
    pk_tt, pk_ll, pk_lt = combine_field_spectra_k3D_lcv(bias, f_growth, D, power_lin_fns, power_rsd_tr_fns, nmesh, Lbox, R, rec_algo)

    # project 3D power spectra to multipoles
    pk_lt_proj = project_3d_to_poles(k_bins, pk_lt, Lbox, poles)[0].reshape(len(poles), len(k_binc))/Lbox**3
    del pk_lt; gc.collect() # noqa: E702
    pk_tt_proj = project_3d_to_poles(k_bins, pk_tt, Lbox, poles)[0].reshape(len(poles), len(k_binc))/Lbox**3
    pk_ll_proj = project_3d_to_poles(k_bins, pk_ll, Lbox, poles)[0].reshape(len(poles), len(k_binc))/Lbox**3

    # expand multipole to 3D power spectra (this is the C-mu_C part)
    assert np.min(np.diff(kth)) == np.max(np.diff(kth)), "For custom interpolation, need equidistant k-values"
