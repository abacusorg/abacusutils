...
def read_power_dict(power_tr_dict, power_m_dict, power_ij_dict, want_rsd, keynames, poles, cross):
    """
...
def run_zcv(power_rsd_tr_dict, power_rsd_m_dict, power_rsd_ij_dict, power_tr_dict, power_m_dict, power_ij_dict, config, cross):
    """
    ...
    k, mu, pk_tt_real, pk_tm_real, pk_mm_real, pk_ij_zz_real, pk_ij_zt_real, pk_ij_zm_real, nmodes = read_power_dict(
        power_tr_dict,
        power_m_dict,
        power_ij_dict,
        want_rsd=False,
    ...
    k, mu, pk_tt_poles, pk_tm_poles, pk_mm_poles, pk_ij_zz_poles, pk_ij_zt_poles, pk_ij_zm_poles, nmodes = read_power_dict(
        power_rsd_tr_dict,
        power_rsd_m_dict,
        power_rsd_ij_dict,
        want_rsd=want_rsd,
    ...
    if cross:
        ...
        pk_zz_cross = combine_cross_spectra(k_binc, pk_ij_zz_input[idx_cross,:], bias_vec[1:], rsd=want_rsd)
        pk_zenbu_cross = combine_cross_spectra(k_binc, pk_ij_zenbu[idx_cross,:], bias_vec[1:], rsd=want_rsd)
        pk_nz_gm = pk_ij_zt_input[0,:]
        pk_zn_gm = combine_cross_spectra(k_binc, pk_ij_zm_input, bias_vec[1:], rsd=want_rsd)
        ...
        for i in range(beta_smooth.shape[0]):
            try:
                beta_smooth_cross[i, :] = savgol_filter(beta_damp_cross.T[:, i], sg_window, 3)
            except ValueError:
                warnings.warn('This message should only appear when doing a smoke test.')

        pk_nn_betasmooth_cross = pk_tm_input - beta_smooth_cross * (pk_zz_cross - pk_zenbu_cross)
